...
function isSameRoute(
  ...
  const param1 = comparedRoute.params || {};
  const query1 = comparedRoute.query || {};
  const name1 = comparedRoute.name;
  const param2 = compareRoute.params || {};
  const query2 = compareRoute.query || {};
  const name2 = compareRoute.name;
...
function arrageObjToTree(
  ...
  const arr: ParentObjINF[] = [];
  const parentArr = obj[parentId];
  if (parentArr && parentArr.length > 0) {
    parentArr.forEach(v => {
      const id: number = v.id as number;
      const idArr = obj[id];
 ...
 */
function arrageDataToTree(arr: ParentObjINF[], parentId = 0): ParentObjINF[] {
...
function gradeChildren(list: GradeChildrenObjINF[]): string[][] {
  const nodes: string[][] = [];
  if (list) {
    const stack: GradeChildrenObjINF[] = list.map(v => v);
    // 广度优先遍历BFS
